...
class TestCvxpyLayer(unittest.TestCase):
    ...
    def test_least_squares(self):
        ...
        self.assertAlmostEqual(
            jnp.linalg.norm(grad_A_cvxpy - grad_A_lstsq).item(), 0.0, places=6
        )
        self.assertAlmostEqual(
            jnp.linalg.norm(grad_b_cvxpy - grad_b_lstsq).item(), 0.0, places=6
        )
    ...
    def test_least_squares_custom_method(self):
        ...
        prob = cp.Problem(cp.Minimize(obj))
        prob_jax = CvxpyLayer(
            prob, [A, b], [x], custom_method=(forward_numpy, backward_numpy)
        )
    ...
    def test_entropy_maximization(self):
        ...
        A_jax, b_jax, F_jax, g_jax = map(
            lambda x: jnp.array(x), [A_np, b_np, F_np, g_np]
        )
    ...
    def test_not_enough_parameters_at_call_time(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "An array must be provided for each CVXPY parameter.*"
        ):
            lam_jax = jnp.ones(1)
            layer(lam_jax)
    ...
    def test_broadcasting(self):
        ...
        self.assertAlmostEqual(
            jnp.linalg.norm(grad_A_cvxpy / 2.0 - grad_A_lstsq).item(), 0.0, places=6
        )
        self.assertAlmostEqual(
            jnp.linalg.norm(grad_b_cvxpy[0] - grad_b_lstsq).item(), 0.0, places=6
        )
